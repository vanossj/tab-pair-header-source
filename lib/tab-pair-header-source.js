...
export default {
  ...
  updateConfig() {
    ...
    if (useAdvancedMatch) {
      ...
      for(let index=0; index < pairExtensions.length; index++) {
        ...
        regstr += "\\." + pairExtensions[index].toLowerCase() + "|";
      }
      regstr[regstr.lenght-1] = ")";
      this.pairRegExp.push( regstr) );
    }
  },
  
  isPair(curr_path, other_paths) {
    //coerce other_paths to an array
    if !Array.isArray(other_paths) {
      if (typeof other_paths === 'string' ||  other_paths instanceof String) {
    ...
    matches = Array(other_paths.length).fill(false)
    for(let index=0; index < this.pairRegExp.length; index++) {
      let curr_match = this.pairRegExp[index].exec(curr_path)
      if (curr_match !== null)
      {
        let curr_match_str = JSON.stringify(curr_match.slice(1))
        for(let jndex=0; jndex < other_paths.length; jndex++) {
          let other_match = this.pairRegExp[index].exec(other_paths[jndex])
          if (other_match !== null && curr_match_str === JSON.stringify(other_match.slice(1)))
          {
            matches[jndex] = true;
          }
        }
  ...
  getPathsForPane(pane) {
    ...
    let item_paths = Array(paneitems.length).fill('');
    for (paneitem_index in paneitems) { // TODO: verify, this assumes that getItems returns items in same order as they are displayed
      paneitem = paneitems[paneitem_index];
      if (atom.isTextEditor(paneitem)) {
        item_paths[pane_index] = paneitem.getPath();
      }
    }
  },
  ...
  checkEverything() {
    ...
    for (let pane_index in panes) {
      ...
      pane_paths = getPathsForPane(pane);
