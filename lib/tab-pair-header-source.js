    ...
    if (useAdvancedMatch) {
      ...
      for(let index=0; index < pairPatterns.length; index++) {
        try {
          ...
          let num_groups = (new RegExp(regex.toString() + '|')).exec('').length - 1;
          if (num_groups > 0) {
            this.pairRegExp.push( regex );
          }
          else {
            no_capture_groups = true;
          }
        }
        catch(err) {
          if (atom.inDevMode()) {
      ...
      if (editor !== null) {
        if ( !invalid_regexp) {
          ...
          if (no_capture_groups) {
            editor.classList.add('highlight-warning');
          }
        }
        if (!no_capture_groups) {
          editor.classList.remove('highlight-warning');
        }
      }
    }
