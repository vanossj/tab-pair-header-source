...
export default {
  ...
  groupPair(paths) {
    ...
    var group_num = 1;
    for(let jndex=0; jndex < paths.length; jndex++) {
        let p = paths[jndx];
        if (matching_regex === null) {
          ...
          if (reg_result !== null) {
            if (matching_stringify === JSON.stringify(reg_result.slice(1))) {
              ...
              matching_regex = res.matching_regex;
              matching_stringify = res.matching_stringify;
              if (matching_regex !== null) { //TODO: move this portion into findMatch()
                groups[jndex] = group_num;
              }
            }
          }
  ...
  findMatch(search_str) {
    matching_regex = null;
    matching_stringify = null;
    for(let index=0; index < this.pairRegExp.length; index++) {
      let reg_result = this.pairRegExp[indx].exec(search_str);
      if (reg_result !== null) {
        matching_regex = this.pairRegExp[indx];
