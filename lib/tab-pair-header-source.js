...
export default {
  ...
  updateConfig() {
    ...
    this.useAdvancedMatch = atom.config.get('tab-pair-header-source.advancedMatch');
    ...
    for(let index=0; index < pairPatterns.length; index++) {
      this.pairRegExp[index] = RegExp(pairPatterns[index]);
    }
  },
  
  isPair(path1, path2) {
    for(let index=0; index < this.pairRegExp.length; index++) {
      var match1 = this.pairRegExp[index].exec(path1)
      var match2 = this.pairRegExp[index].exec(path2)
      if (match1 !== null && match2 !== null)
      {
        return JSON.stringify(match1.slice(1)) === JSON.stringify(match2.slice(1))
      }
    }
    return false;
  },
